...
public class APIController {
	...
	public @ResponseBody String calcSplines(@RequestBody String message) {
        ...
        points.set(0, 
            new Pose2d(
                new Translation2d(points.get(0).getTranslation()), 
                new Rotation2d(
                    points.get(1).getTranslation().x() - points.get(0).getTranslation().x(),
                    points.get(1).getTranslation().y() - points.get(0).getTranslation().y(),
                    true
                )
            )
        );
        for (int i=1; i<points.size()-1; i++) {
            points.set(i, 
                new Pose2d(
                    new Translation2d(points.get(i).getTranslation()), 
                    new Rotation2d(
                        points.get(i+1).getTranslation().x() - points.get(i-1).getTranslation().x(),
                        points.get(i+1).getTranslation().y() - points.get(i-1).getTranslation().y(),
                        true
                    )
                )
        );}
        points.set(points.size()-1, 
            new Pose2d(
                new Translation2d(points.get(points.size()-1).getTranslation()), 
                new Rotation2d(
                    points.get(points.size()-1).getTranslation().x() - points.get(points.size()-2).getTranslation().x(),
                    points.get(points.size()-1).getTranslation().y() - points.get(points.size()-2).getTranslation().y(),
                    true
                )
            )
        );
